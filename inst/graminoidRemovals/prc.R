...
env.var <- forbcom %>%
  ...
  #filter(TTtreat == "RTC") %>%
  filter(Temperature_level == 10.5) %>%
  select(wmean_height, wmean_leafSize, wmean_seedMass, wmean_maxheight, wmean_minheight, sumcover, wmean_LDMC_global, wmean_SLA_global, wmean_CN_global, wmean_LTH_global, wmean_LA_global, wmean_height_global, richness, diversity, evenness, functionalgroup, TTtreat, as.numeric(Year), summer_temp, annPrecip)
  
x <- rda(forbcover ~  Year + TTtreat + annPrecip, data = env.var) # can't put both site and explan variables into analyses wmean_leafSize+ wmean_seedMass+ sumcover+ wmean_LDMC_global+ wmean_SLA_global+ wmean_CN_global+ wmean_LTH_global+ wmean_LA_global + wmean_height_global + 
