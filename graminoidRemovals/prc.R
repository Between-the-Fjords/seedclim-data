...
w <- ulv %>% mutate(turfID = factor(turfID)) %>% group_by(siteID) %>% 
  do({
    spp <- xtabs(cover ~ ID + species, data = .)
    spp <- as.data.frame(unclass(spp))
    ...
    scores <- as.data.frame(c(scores, prc1$CCA$biplot[,2]))
  })
...
abundPlot <- my.GR.data %>% 
  filter(functionalGroup == "forb", temp > 10) %>% 
  group_by(siteID, Year, species) %>% 
  mutate(meanCov = mean(cover)) %>% 
  ungroup() %>% 
  group_by(siteID, species) %>%
  mutate(dominance = if_else(min(meanCov, na.rm = TRUE) > 10, "dominant", "subordinate")) %>% 
...
ggsave(filename = paste0("fig4.jpg"), width = 11, height = 4.5, dpi = 300, path = "/Users/fja062/Documents/seedclimComm/figures")
