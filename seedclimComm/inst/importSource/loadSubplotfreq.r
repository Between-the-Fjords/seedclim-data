...
#fat       
subturf<-xtabs(rep(1, nrow(subturf.thin))~paste(turfID, subTurf, Year, sep="_") + species,
               data = subturf.thin)
...
boxplot(rowSums(fsubturf) ~ recorder + Temperature_level, data = cover.meta, notch=TRUE, las=2)
boxplot(rowSums(fsubturf > 0) ~ recorder + Temperature_level, data = cover.meta, notch=TRUE, las=2)
