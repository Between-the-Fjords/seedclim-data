...
Biomass_Data_Type <- read.table(header = TRUE,
                                ...
                                text = "Old_Biomass_Attr Biomass_Attributes Data_Type Units
                                  SH_B Highest_shoot Raw g
                                  PH_B Highest_flowering_Shoot_with_stem Raw g
                                  PH0stem Flowering_Shoot_without_stem Raw g
                                  LL Largest_Leaf Raw g
                                  SStem.T Shoots_with_Leaves Raw g
                                  PStem.T Shoots_with_Flowers Raw g
                                  Tu.Rt Tuber_with_Roots Raw g
                                  Tuber Tuber Raw g
                                  St.Rt Stolen_with_Roots Raw g
                                  Stolon Stolon Raw g
                                  rhiz. Rhizome Raw g
                                  roots Roots Raw g
                                  stems Stems Raw g
                                  leaves Leaves Raw g
                                  fruits Fruits Raw g
                                  TbiomR Total_Biomass_per_Ramet Raw g
                                  Tbiom Total_Biomass_per_Genet Raw g
                                  GG.1 Generative_Growth Raw g
                                  ag Above_Ground Raw g
                                  agr Above_Ground_and_Reproductive Raw g
                                  bg Below_Ground Raw g
                                  r Reproductive Raw g
                                  agbg Above_w_Below_Ground Raw g
                                  agrbg Above_Ground_with_Reproductive_and_Below Raw g
                                  RBM Root_with_Rhizome_per_ramet Calculated mg
                                  SBM Stem_per_genet Calculated mg
                                  LBM Leaf_per_genet Calculated mg
                                  FBM Reproductive_per_genet Calculated mg
                                  BM Total_Sterile_per_genet Calculated mg
                                  BMTOT Total_per_genet Calculated mg
                                  AB Above_Below_Ground_ramet_ratio Calculated NA
                                  SR Stem_Below_Ground_ramet_ratio Calculated NA
                                  LR Leaf_Below_Ground_ramet_ratio Calculated NA
                                  FV Fertile_Sterile_ramet_ratio Calculated NA
                                  FL Fertile_Photosynthetic_ramet_ratio Calculated NA
                                  BioDL Dead_Leaves Raw g
                                  BioSS Sterile_Stems Raw g
                                  BMseeds Biomass_Seeds Calculated g
                                  "
)
...
SG8_Biomass <- Biomass_Combined_long_cleaned %>% 
  select(year, month, siteID, genet = IDG, ramet = IDS, mother_shoot = MS, individualID = IND, 
         species, biomass_attributes = Biomass_Attributes, value = Values, units = Units, data_type = Data_Type, 
...
ggplot(SG8_Biomass, aes(x = species, y = value, colour = siteID)) +
  geom_point() +
  facet_grid(biomass_attributes ~ Units, scales = "free_y") +
