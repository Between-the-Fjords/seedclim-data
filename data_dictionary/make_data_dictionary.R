...
make_data_dictionary <- function(data, description_table, table_ID, keep_table_ID = FALSE){
  ...
  class <- map_dfr(data %>% as_tibble, ~enframe(class(.x), value = "Variable type"), 
                   .id = "Variable name") %>% 
    select(-name) %>% 
    # give sensible names
  ...
  dictionary <- bind_rows(
    ...
    select(TableID, "Variable name", Description, "Variable type", "Variable range or levels", "Units/treatment level", "How measured")
